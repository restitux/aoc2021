...
fn do_fold(map: Map, fold: &Instruction) -> Map {
    match fold.axis {
        Axis::X => {
            let mut new_map = new_map((map.x / 2) + 1, map.y);
        ...
        Axis::Y => {
            let mut new_map = new_map(map.x, (map.y / 2) + 1);
...
fn main() -> Result<()> {
    ...
    let mut count = count_dots(&map);

    println!("COUNT: {}", count);

    map = do_fold(map, &instructions[0]);

    print_dim(&map);
    //print_map(&map);

    count = count_dots(&map);

    println!("COUNT: {}", count);

    map = do_fold(map, &instructions[1]);
