...
fn main() -> Result<()> {
    ...
    let mut valid_points: HashMap<(i64, i64), i64> = HashMap::new();
    ...
    'outer: for x in min_x_val..max_x_val {
        ...
        for step in valid_steps {
            if step != x {
                ...
                let max_y = (step / 2) + 1;
                println!("MAX_Y: {}", max_y);
                for y in y_bounds.1..y_bounds.1.abs() {
                    let mut pos = 0;
                    ...
                    if pos >= y_bounds.1 && pos <= y_bounds.0 {
                        println!("A Valid Trajectory: {}, {} -> {}", x, y, max_height);
                        if max_height > total_max_height {
                            total_max_height = max_height;
                        }
                        valid_points.insert((x, y), max_height);
                ...
                for y in y_bounds.1..y_bounds.1.abs() {
                    ...
                    loop {
                        ...
                        if pos >= y_bounds.1 && pos <= y_bounds.0  && steps >= x {
                            println!("B Valid Trajectory: {}, {} -> {}", x, y, max_height);
                            if max_height > total_max_height {
                                total_max_height = max_height;
                            }
                            valid_points.insert((x, y), max_height);
    ...
    for ((x, y), max_height) in valid_points.iter() {
        if max_height > &total_max_height {
            total_max_height = max_height.clone();
        }
    }

    println!("MAX HEIGHT: {}", total_max_height);
    println!("valid_points.len(): {}", valid_points.len());
